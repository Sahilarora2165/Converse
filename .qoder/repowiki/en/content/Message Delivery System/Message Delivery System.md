...
## Introduction
This document explains the message delivery system built on an event-driven architecture with robust message state management. It covers the message state machine (SENT, DELIVERED, SEEN), delivery and seen acknowledgment mechanisms, Kafka integration for asynchronous processing, WebSocket broadcasting, database persistence, and real-time notifications. It also documents configuration options for Kafka topics, message retention, and processing guarantees, and addresses common reliability concerns such as ordering, duplicate processing, and failure recovery.

## Project Structure
The message delivery system spans several layers:
...
- Kafka producer/consumer orchestrate asynchronous processing.
...
The system separates concerns across REST, asynchronous processing, and real-time updates:
...
Send --> Broker["Kafka Broker Partitions"]
Broker --> Consume["KafkaListener consumes event"]
Consume --> Save["MessageService.sendMessage(...)"]
...
- Consumer errors: Exceptions are logged and retried according to error handler configuration.
...
## Conclusion
The message delivery system enforces a strict, server-authoritative state machine with robust acknowledgments, Kafka-backed asynchronous processing, and real-time WebSocket updates. It prioritizes correctness, crash safety, and scalability while remaining comprehensible for both beginners and advanced developers.
...
- Message retention: controlled by Kafka cluster/topic retention policies (not defined in code).
