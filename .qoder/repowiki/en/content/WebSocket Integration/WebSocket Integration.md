...
## Update Summary
**Changes Made**
- Enhanced STOMP configuration documentation with detailed endpoint specifications and authentication flow
- Expanded real-time features section with comprehensive message delivery and read receipt implementations
- Added detailed frontend WebSocket client implementation with connection lifecycle management
- Updated message routing mechanisms with explicit endpoint mappings and subscription patterns
- Improved configuration options documentation with environment variable support
- Added practical examples from React frontend showing WebSocket integration patterns
...
## Introduction
This document explains the WebSocket integration for real-time communication in the Chatify application. It covers STOMP protocol configuration, WebSocket endpoint setup, message routing, and the end-to-end message handling architecture. The system implements comprehensive real-time features including message delivery receipts, seen/read receipts, typing indicators, and online presence tracking. The documentation includes practical examples from the React frontend showing client-side connection lifecycle management, subscription handling, and message publishing with detailed endpoint specifications and configuration options.

## Project Structure
The WebSocket integration spans two layers:
- Backend (Spring Boot): WebSocket broker configuration, STOMP endpoint registration, inbound message handlers, and outbound broadcasting via a message broker.
...
CONST["constants.js"]
CHAT["Chat.jsx<br/>Real-time message handling"]
end
...
EVENT["ChatMessageEvent.java<br/>Message event structure"]
end
WSCTX --> CHAT
HOOK --> WSCTX
CONST --> WSCTX
...
- [Chat.jsx:34-287](file://chatify-frontend/src/pages/Chat.jsx#L34-L287)
...
- Backend WebSocket configuration and authentication:
  - STOMP endpoint registration with SockJS fallback at "/ws" with configurable allowed origins.
...
CTRL-->>BROKER : "Broadcast via /topic/chatroom/{id}"
...
- Endpoint registration:
  - Adds a WebSocket endpoint at "/ws" with SockJS enabled and configurable allowed origins via CORS configuration.
- Message broker:
...
- Message send:
  - Two endpoints: legacy "/chat.sendMessage" and primary "/chat/{roomId}/sendMessage". Both validate membership and publish to Kafka.
- Delivery receipts:
...
- Backend
  - Allowed origins for WebSocket endpoint via CORS configuration.
...
**Section sources**
- [WebSocketContext.jsx:124-129](file://chatify-frontend/src/context/WebSocketContext.jsx#L124-L129)
- [ChatMessageEvent.java:16-25](file://src/main/java/com/chatify/chat_backend/dto/ChatMessageEvent.java#L16-L25)
